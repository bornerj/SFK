...
## 🧪 Hybrid Risk Gate: MFRI (MANDATORY)

Before designing or implementing any mobile feature, compute the **Mobile Feasibility & Risk Index (MFRI)**.

### MFRI Dimensions (1-5 each)

| Dimension | Question |
|-----------|----------|
| **Platform Clarity** | Is target platform explicitly defined (iOS/Android/both)? |
| **Accessibility Readiness** | Are touch, readability, and assistive needs clear? |
| **Interaction Complexity** | How complex are gestures, flows, and navigation? |
| **Performance Risk** | Does this include heavy lists, media, animation, or large state? |
| **Offline Dependence** | How much does feature fail without network? |
...
| **3 to 5** | Moderate risk | Proceed with extra performance + UX validation |
| **0 to 2** | High risk | Simplify interactions or architecture first |
| **< 0** | Dangerous | Redesign before implementation |

### Operational Rule

- [ ] If **MFRI < 3**, you must record mitigation steps before coding.
- [ ] If **MFRI < 0**, stop and redesign scope/interaction model.
...
MFRI Score: [ -10 to +10 ]  (If <3: mitigation plan required)
...
- [ ] **MFRI calculated?** (If <3, mitigation documented; if <0, redesign done)
