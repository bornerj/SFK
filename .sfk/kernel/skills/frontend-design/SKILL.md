...
## 🧭 Hybrid Quality Gate: DFII (MANDATORY)

Before implementation, score the chosen direction with the **Design Feasibility & Impact Index (DFII)**.

### DFII Dimensions (1-5 each)

| Dimension | Question |
|-----------|----------|
| **Aesthetic Impact** | Is this direction visually distinctive and memorable? |
| **Context Fit** | Does it fit product, audience, and task? |
| **Implementation Feasibility** | Can we implement it cleanly with current stack/time? |
| **Performance Safety** | Will it remain accessible and fast? |
| **Consistency Risk** | How hard is it to keep this style consistent? |
...
| **4-7** | Reduce visual scope/effects |
| **<=3** | Rethink direction |

### Rule

- [ ] **Do not execute visual-heavy direction when DFII < 8 without explicit user approval.**
...
### Differentiation Anchor (MANDATORY)

Define one explicit, memorable signature before building:

> "If this UI is screenshotted without logo/text, what still makes it recognizable?"

Examples:
- distinct typography rhythm
- asymmetric composition system
- unique CTA treatment
- signature motion sequence on entry

- [ ] At least one anchor must appear in final output.
...
### Anti-Default Font Rule

- Avoid defaulting to Inter/Roboto/Arial/system stack.
- If a default/system font is required (brand, product constraints), state the reason explicitly.
...
- [ ] Moments of delight
- [ ] One memorable "signature moment" users can recall later
...
## 11. Required Delivery Format (for build-ready output)

When producing frontend work, return in this structure:

1. **Design Direction Summary**
   - aesthetic name (clear style direction)
   - DFII score with quick rationale
   - target audience and emotional goal
2. **Design System Snapshot**
   - typography choices + rationale
   - CSS color variables
   - spacing rhythm + layout logic
   - motion philosophy
3. **Implementation**
   - production-ready code (HTML/CSS/JS or framework)
